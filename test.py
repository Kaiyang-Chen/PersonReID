...
parser.add_argument('--method',default='static', type=str,help='static or dynamic correlation')
    ...
    for i in range(class_num):
        ...
        for j in range(6):
            ...
            if list1[j+1][1] != 0:
                ...
                #     print(s)
                if start == 0:
                    res[des][int(list1[j][1]/110000)] += 1
...
def tracking(split, s_threshold, t_threshold,index):
    ...
    # totals = []
    # misses = []
    spatial_temporal_distribution(train_cam, train_label, train_frames, train_feature_paths, 0, 440000)
    for tmp_sp in split:
        break
        test_itr = 750
        ...
        for i in range(test_itr):
            ...
            model_path = './correlation_predict_56.mat'
            result = scipy.io.loadmat(model_path)
            ...
            temporal_distribution = temporal_distribution_start
            ...
            tmp_split = int(cu_split)
            seq = int(tmpFrame*tmp_sp/440000/tmp_split)
            tmp = spatial_distribution[int(seq*tmp_split)]
            flag  = False
            for k in range(int(seq*tmp_split+1), int(seq*tmp_split+56/tmp_split + 1)):
                # print("check\n")
                # print(tmp[0])
                if not flag:
                    tmp_spatial = [[tmp[i][j]/(56/tmp_split) + spatial_distribution[k][i][j]/(56/tmp_split)  for j in range(len(tmp[0]))] for i in range(len(tmp))]
                    flag = True
                else:
                    tmp_spatial = [[tmp[i][j] + spatial_distribution[k][i][j]/(56/tmp_split)  for j in range(len(tmp[0]))] for i in range(len(tmp))]
                tmp = tmp_spatial

            # tmp_spatial = [[tmp[i][j]/(32/tmp_split)  for j in range(len(tmp[0]))] for i in range(len(tmp))]
            # print((32/tmp_split))
            # tmp_spatial =np.divide(tmp,(32/tmp_split))
            # print(tmp_spatial)
            matched_predicted, queryNum_predicted = priorityTracking(tmpObj, int(tmpFrame), tmpCamera, feat_q , tmp_spatial, temporal_distribution, reid_method, s_threshold, t_threshold,True)
            # model_path = './correlation_split' + str(tmp_sp) + '_' + str(seq) + '.mat'
            # result = scipy.io.loadmat(model_path)
            # spatial_distribution = result['spatial']
            # # temporal_distribution = result['temporal']
            # matched_dynamic, queryNum_dynamic = priorityTracking(tmpObj, int(tmpFrame), tmpCamera, feat_q , spatial_distribution, temporal_distribution, reid_method)
            # model_path = './correlation_split' + str(int(tmp_sp/2)) + '_0.mat'
            # result = scipy.io.loadmat(model_path)
            # spatial_distribution = result['spatial']
            # # temporal_distribution = result['temporal']
            # matched_static, queryNum_static = priorityTracking(tmpObj, int(tmpFrame), tmpCamera, feat_q , spatial_distribution, temporal_distribution, reid_method)
            ...
            # print(matched_dynamic)
            # print(queryNum_dynamic)
            ...
            #     total_static += queryNum_static
            #     static += queryNum_static
            #     commonTotal_static += commonQueryNum
            # else:
            #     if len(matched_static) == 0:
            #         total_static += queryNum_static
            #         total_static += commonQueryNum
            #         static_miss += 1
            #         commonTotal_static += commonQueryNum
            #     else:
            #         static_err += 1
            # if matched_dynamic == commonMatched:
            #     total_dynamic += queryNum_dynamic
            #     dynamic += queryNum_dynamic
            #     commonTotal_static += commonQueryNum
            # else:
            #     if len(matched_dynamic) == 0:
            #         total_dynamic += queryNum_dynamic
            #         total_dynamic += commonQueryNum
            #         dynamic_miss +=1
            #         commonTotal_static += commonQueryNum
            #     else:
            #         dynamic_err += 1
            ...
            if common_obj == tmpObj:
                ...
                # if our method does not find the correct answer    
                    ...
                    if len(matched_predicted) == 0:
                        matched_predicted, queryNum_predicted_tmp = priorityTracking(tmpObj, int(tmpFrame), tmpCamera, feat_q , tmp_spatial, temporal_distribution, reid_method, s_threshold, t_threshold, False)
            ...
            print(" predict_correct: " + str(predict_correct) + " common_correct: " + str(common_correct) + " predict_retrieved: " + str(predict_retrieved)+ " common_retrieved: " + str(common_retrieved) + '\n')
        ...
        print("predict method_split" + str(tmp_sp) + " spatial threshold: " +str(s_threshold)+ " temporal threshold: " +str(t_threshold)+ " recall: " +str(float(predict_correct/750))+ " precision: " +str(float(predict_correct/predict_retrieved)) + " searched frames: " + str(total_predict) + "\n")
        fo.write("predict method_split" + str(tmp_sp) + " spatial threshold: " +str(s_threshold)+ " temporal threshold: " +str(t_threshold)+ " recall: " +str(float(predict_correct/750))+ " precision: " +str(float(predict_correct/predict_retrieved)) + " searched frames: " + str(total_predict) + "\n")
        print("less predict method_split" + str(tmp_sp) + " spatial threshold: " +str(s_threshold)+ " temporal threshold: " +str(t_threshold)+ " recall: " +str(float(predict_correct_less/total_query))+ " precision: " +str(float(predict_correct_less/predict_retrieved_less)) + " searched frames: " + str(total_predict_less) + "\n")
        fo.write("less predict method_split" + str(tmp_sp) + " spatial threshold: " +str(s_threshold)+ " temporal threshold: " +str(t_threshold)+ " recall: " +str(float(predict_correct_less/total_query))+ " precision: " +str(float(predict_correct_less/predict_retrieved_less)) + " searched frames: " + str(total_predict_less) + "\n")
...
def main():
    ...
    #     in_split = [56,60]
    in_split = [i]
    tracking(in_split, s_threshold, t_threshold, index)
